...
/// 在地址TRAMPOLINE下按照给定的kstack id寻找到对应的kstack的top和bottom的虚拟地址
...
/// 在kernel的虚拟地址空间（TRAMPOLINE下面）中分配一个给线程用的kstack，过程中为这个kstack分配对应的物理页。
/// 最后返回对应的kernel_id
...
impl KernelStack {
    ...
    /// 返回本kstack的top(User的kstack,不是kernel的)
...
impl TaskUserRes {
    /// 为本线程初始化TaskUserRes:
    /// 1. 新建TaskUserRes，
    /// 2. 为线程对应的user stack分配物理页（本进程对应的地址空间中），
    /// 3. 为线程对应的TrapContext存放点分配物理页（本进程对应的地址空间中）
    pub fn new(
    ...
    /// 给每一个本线程对应的user stack分配物理页（大小为一个物理页，
    /// 同时，给本线程存放TrapContext的地方分配物理页
